...
def inject_dob_anomalies(df, rate):
    num_to_affect = int(len(df) * rate)  # calculate number of rows to affect
    indices = np.random.choice(df.index, size=num_to_affect, replace=False)  # select random indices
    
    # List of invalid formats requested
    bad_formats = ["1998-15-45", "32/13/2001", "abcd", "2001", "??/??/????", "12-2001", "01/01", "Invalid Date"]  # bad formats
    
    if "DOB" in df.columns:  # check if column exists
        df.loc[indices, "DOB"] = [random.choice(bad_formats) for _ in range(num_to_affect)]  # inject bad formats
...
def inject_diagnosis_logic_violations(df, rate):
    ...
    dependent_cols = ["Cancer_Type", "Stages", "Tumor_Size"]  # columns dependent on diagnosis
    
    # Check if columns exist to avoid errors
    existing_cols = [c for c in dependent_cols if c in df.columns]  # filter existing columns
    
    for idx in indices:  # iterate through selected indices
        if "Diagnosis_Status" not in df.columns: break  # stop if diagnosis column missing
        
        status = df.loc[idx, "Diagnosis_Status"]  # get diagnosis status
        
        # Violation 1: Negative Diagnosis but has data
        if status == "Negative":  # if negative
            for col in existing_cols:  # iterate dependent columns
                # Insert random fake data instead of being empty
                if col == "Tumor_Size":
                    df.loc[idx, col] = random.choice([2.5, 5.0, 12.1])  # inject fake size
                elif col == "Stages":
                    df.loc[idx, col] = random.choice(["I", "II", "III"])  # inject fake stage
                elif col == "Cancer_Type":
                    df.loc[idx, col] = random.choice(["Breast Cancer", "Lung Cancer"])  # inject fake type
                    
        # Violation 2: Positive Diagnosis but missing data
        elif status == "Positive":  # if positive
            for col in existing_cols:  # iterate dependent columns
                df.loc[idx, col] = random.choice([np.nan, "N/A"])  # inject missing data
...
misplacement_map = {  # map for mismatched columns
    "Gender": "Age",
    "Age": "Gender",
    "Blood_Group": "Gender",
...
type_conversion_map = {  # map for wrong types
    "Age": "numeric_to_text",
...
out_of_range_map = {  # map for out of range values
    "Age": [-5, -1, 130, 150],
...
categorical_map = {  # map for invalid categories
...
format_corruption_map = {  # map for format corruption
...
df_anomalous = inject_dob_anomalies(df_anomalous, rate=random.uniform(0.05, 0.08)) # inject bad DOBs

# 8. NEW: Diagnosis Logic Violations
df_anomalous = inject_diagnosis_logic_violations(df_anomalous, rate=random.uniform(0.05, 0.10)) # inject logic errors
